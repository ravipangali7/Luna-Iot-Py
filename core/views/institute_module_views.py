...
@require_auth
@api_response
def get_public_vehicle_institutes(request):
    """
    Get institutes where the current user has access to the public-vehicle module
    For Super Admin: returns all institutes with public-vehicle module enabled
    For other users: returns only institutes where user has access
    """
    try:
        ...
        if is_admin:
            # Super Admin: Get all institutes with public-vehicle module enabled
            all_institute_modules = InstituteModule.objects.filter(
            ...
            for institute_module in all_institute_modules:
                institutes_data.append({
                    'institute_id': institute_module.institute.id,
                    'institute_name': institute_module.institute.name,
                    'has_public_vehicle_access': True
                })
        else:
            # Regular users: Get only institutes where user has access to public-vehicle module
            user_institute_modules = InstituteModule.objects.filter(
                module=public_vehicle_module,
                users=request.user
            ).select_related('institute')
        ...
        return success_response(
            data=institutes_data,
            message=SUCCESS_MESSAGES.get('DATA_RETRIEVED', 'Public vehicle institutes retrieved successfully')
        )
    except Exception as e:
        return error_response(
            message=ERROR_MESSAGES.get('INTERNAL_ERROR', 'Internal server error'),
